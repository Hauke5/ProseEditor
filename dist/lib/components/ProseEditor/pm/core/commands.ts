import { Fragment, NodeType, Slice } 
                                 from 'prosemirror-model';
import { Command, EditorState, NodeSelection, Selection, TextSelection } 
                                 from 'prosemirror-state';
import { ReplaceStep }           from 'prosemirror-transform';
import { CommandQuery }          from '../nodes/common';
...
export function jumpToEndOfNode(type: NodeType): Command {
   return (state:EditorState, dispatch?:Dispatch) => {
      ...
      if (dispatch) {
         ...
         dispatch(state.tr.setSelection(TextSelection.create(state.doc, start??0 + node.content.size)));
      }
      return true;
   };
}
