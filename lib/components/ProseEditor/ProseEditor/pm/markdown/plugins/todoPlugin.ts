...
export function todoListMarkdownItPlugin(md:MarkdownIt, options:Options = {}) {
   const {
   ...
   function processToken(tokens: Token[], index: number) {
      if (tokens[index]?.type !== 'bullet_list_open') return;
      ...
      children.forEach(([todoItem]) => {
         if (todoItem.type === 'list_item_close') todoItem.type = todoItemCloseType;
         if (todoItem.type === 'list_item_open')  todoItem.type = todoItemOpenName;
      })

      children.filter(([todoItem]) => todoItem.type === todoItemOpenName)
         ...
         assert(inlineToken, 'inlineToken cannot be undefined');
         ...
         assert(inlineTokenChild, 'inlineTokenChild cannot be undefined');

         let isDone:'yes'|'no'|null = null;
         ...
         if (startsWithTodoMarkdown(inlineTokenChild)) {
            ...
            isDone = (charAt1 && ['x', 'X'].includes(charAt1)) ? 'yes' : 'no'
         }

         const existingAttrs = todoItem.attrs;

         if (isDone) todoItem.attrs = existingAttrs
            ? [[isDoneAttrName, isDone], ...existingAttrs]
            : [[isDoneAttrName, isDone]]
   ...
   function getChildren(tokens: Token[], position: number) {
      ...
      assert(parentOpen, 'parentOpen cannot be undefined');
