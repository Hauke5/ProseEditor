import { Attrs, DOMOutputSpec, Mark, MarkType, Node, Schema } 
...
import { quote }              from '../core/helpers';
import { StandardCommands }   from '../common';
import { conditionalCommand } from '../nodes/common';
import { MarkDesc }           from './common'

const name = 'link'
const tag  = 'a'


export interface LinkDesc extends MarkDesc {
   queryLinkAttrs: typeof queryLinkAttrs,
   commands: StandardCommands & {
      createLink:             (href:string)=> Command
      updateLink:             (href?: string)=> Command
      isLinkAllowedInRange:   (from: number, to: number)=>Command
      isSelectionAroundLink:  Command
   }
}
                                    
export const link:LinkDesc = {
   type:             'mark',
   name,
   schema: {
      ...
      toDOM: (node): DOMOutputSpec => [
         'a',
         {...node.attrs, rel: 'noopener noreferrer nofollow', title:node.attrs.title ?? node.attrs.href},
         0,
      ],
   },
...
const PASTE_RULE = /https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{2,256}\.[a-zA-Z]{2,}\b([-a-zA-Z0-9@:%_+.~#?&//=]*)/g
...
function pluginsFactory(): RawPlugins {
   return ({ schema }) => {
      ...
      return [
         ...
         markPasteRule(PASTE_RULE, type, (match) => ({ href: match })), new Plugin({
            props: {
...
function getMarkAttrs(editorState: EditorState, type: MarkType):Attrs {
...
// same, but with any string that has one or more parts separated by a . 
const URL_REGEX = /(^|\s)(((http|https|ftp):\/\/)?(?:[^\s.:\/]+\.)+(?:\w+))\s$/

/** automatically inserts a link when typing a complete URL, e.g. http://mydomain.com */
function autoLinkInputRule(type: MarkType) {
   return new InputRule(URL_REGEX, (state, match, start, end) => {
      ...
      const href = text // scheme ? text : `http://${text}`;
      const tr = state.tr;
      tr.addMark(
...
function linkInputRule(type: MarkType) {
   const INPUT_EXP = /(^|\s)\[([^\]]+)\]\(((http|https|ftp):\/\/)?([^\)]+)\)(\s|$)/
   return new InputRule(INPUT_EXP, (state, match, start, end) => {
      if (!match[0]) return null
...
function pasteLink(regexp: RegExp) {
   return new Plugin({
      props: {
...
class MatchType {
   constructor(public start: number, public end: number, public match: boolean, private sourceString: string) {}
   get subString() {
...
function markPasteRule(regexp: RegExp, type: MarkType, getAttrs: Mark['attrs'] | ((x: string) => Mark['attrs'])) {
   return new Plugin({
      props: {
...
function setLink(from: number, to: number, href?: string) {
   ...
   return conditionalCommand(
      ...
      (state, dispatch) => {
         ...
         if (href) {
            const mark = getTypeFromSchema(state.schema).create({href: href});
 ...
 * @param {*} href
 */
function createLink(href: string):Command {
   return conditionalCommand(
      (state) =>isLinkAllowedInRange(state.selection.$from.pos, state.selection.$to.pos)(state),
      (state, dispatch) => {
...
function updateLink(href?: string): Command {
...
function isSelectionAroundLink(state: EditorState) {
   const { $from, $to } = state.selection;
   const node = $from.nodeAfter;

   return !!node && $from.textOffset === 0 &&
