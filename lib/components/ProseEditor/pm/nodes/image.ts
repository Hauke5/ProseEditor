...
import { Command, NodeSelection, Plugin, PluginKey } 
...
interface PluginProps extends PluginsFactoryProps{
   handleDragAndDrop?: boolean;
   acceptFileType?: string;
   createImageNodes?: (files: File[], imageType: NodeType, view: EditorView) => Promise<Node[]>;
}

function pluginsFactory({handleDragAndDrop = true, acceptFileType = 'image/*', createImageNodes = defaultCreateImageNodes}: PluginProps = {}): RawPlugins {
   return ({ schema }) => {
            ...
            props: {
               handleDOMEvents: {
                  drop(view, _event) {
                     ...
                     createImageNodes(files, getNodeType(view.state, name), view)
                        .then((imageNodes) => {
...
const updateImageNodeAttribute =
   (attr: Node['attrs'] = {}): Command =>
      (state, dispatch) => {
         if (!(state.selection instanceof NodeSelection) || !state.selection.node) return false
         const { node } = state.selection;
         if (node.type !== getNodeType(state, name)) return false

         if (dispatch)
            dispatch(state.tr.setNodeMarkup(state.selection.$from.pos, undefined, { ...node.attrs, ...attr}))
         return true;
      };
