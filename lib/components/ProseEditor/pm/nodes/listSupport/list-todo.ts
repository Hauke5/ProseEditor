...
import { EditorState, Transaction }
                        from 'prosemirror-state';
import { InputRule }    from 'prosemirror-inputrules';
import { canJoin, findWrapping }         
                        from 'prosemirror-transform';
import { conditionalCommand, getNodeType }  
                        from '../common';

export const isNodeTodo = (node: Node, schema: Schema) => (
   node?.type === getNodeType(schema, 'listItem') && typeof node?.attrs['todoChecked'] === 'boolean'
)
 ...
 */
export const removeTodo = conditionalCommand(
 ...
 */
export const setTodo = conditionalCommand(
   [isSelectionParentBulletList, (state) => {
      ...
      let { tr, schema } = state;
      siblingsAndNodesBetween(state, (node, pos) => tr = setTodoCheckedAttr(tr, schema, node, pos))
      if (dispatch) dispatch(tr)
      return true
   }, 
);
...
export function wrappingInputRuleForTodo(regexp: RegExp, getAttrs: Node['attrs'] | ((match: RegExpMatchArray) => Node['attrs'])) {
   return new InputRule(regexp, function (state, match, start, end) {
      ...
      if (before?.type === getNodeType(state, 'bulletList') &&
 ...
 *    - nodes that are siblings of the top level nodes
...
export function siblingsAndNodesBetween(state: EditorState, callback: (node: Node, pos: number) => void) {
   ...
   const range = $from.blockRange($to, (node) =>
      node.childCount > 0 && node.firstChild!.type === getNodeType(schema, 'listItem'),
   );
   if (!range) return
...
function isSelectionParentBulletList(state: EditorState) {
   ...
   return (fromNode?.type === getNodeType(state, 'bulletList') &&
...
function todoCount(state: EditorState) {
   ...
   siblingsAndNodesBetween(state, (node, _pos) => {
      ...
      if (node.type === getNodeType(schema, 'listItem')) lists++;
      if (isNodeTodo(node, schema)) todos++;
   });
