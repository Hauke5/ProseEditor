...
import { mapChildren }        from '../../core/helpers'
import { mapSlice }           from '../../core/transforms';
import { getNodeType }        from '../common';

// This will return (depth - 1) for root list parent of a list.
const getListLiftTarget = (type: NodeType | null | undefined, schema: Schema, resPos: ResolvedPos): number => {
   let target = resPos.depth;
   const { bulletList, orderedList } = schema.nodes;
   let listItem = type;
   if (!listItem) ({ listItem } = schema.nodes)

   for (let i = resPos.depth; i > 0; i--) {
      const node = resPos.node(i);
      if (node.type === bulletList || node.type === orderedList) target = i
      if (node.type !== bulletList && node.type !== orderedList && node.type !== listItem) break
   }
   return target - 1;
};

function liftListItem(type: NodeType | undefined, state: EditorState, selection: Selection, tr: Transaction) {
   const { $from, $to } = selection;
   const listItem = type ?? getNodeType(state, 'listItem');
   ...
   if (end < endOfList) {
      const slice = new Slice(Fragment.from(listItem.create(undefined, range.parent.copy())), 1, 0)
      tr.step(new ReplaceAroundStep(end-1, endOfList, end, endOfList, slice, 1, true));
...
export function liftFollowingList(type: NodeType | undefined, state: EditorState, from: number, to: number, rootListDepth: number, tr: Transaction) {
   const listItem = type ?? getNodeType(state, 'listItem');
   let lifted = false;
...
export function liftSelectionList(type: NodeType | undefined, state: EditorState, tr: Transaction) {
   ...
   for (let i = listCol.length - 1; i >= 0; i--) {
      ...
      if (start.depth > 0) {
         const end = paragraph.node.textContent && paragraph.node.textContent.length > 0
            ? tr.doc.resolve(tr.mapping.map(paragraph.pos + paragraph.node.textContent.length))
            : tr.doc.resolve(tr.mapping.map(paragraph.pos + 1))
...
const extractListFromParagaph = (type: NodeType | undefined, node: Node, schema: Schema) => {
  const { hardBreak, bulletList, orderedList } = schema.nodes;
  const content = mapChildren(node.content);
  ...
  const listified = content
      .map((child) => {
         ...
         if (!listMatch || !child.text) return child

         const [nodeType, length] = listMatch;
         // convert to list item
         const newText = child.text.substr(length);
         const listItem = type ?? getNodeType(schema, 'listItem')

         const theType = getNodeType(schema, 'paragraph').createChecked(undefined, newText.length ? schema.text(newText) : undefined)
         const listItemNode = listItem.createAndFill(undefined, theType)
         if (!listItemNode) return child;
   ...
   // try to join
   const mockState = EditorState.create({schema})
   let lastTr: Transaction | undefined;
   const mockDispatch = (tr: Transaction) => {lastTr = tr}
   autoJoin((state, dispatch) => {
      if (!dispatch) return false
      dispatch(state.tr.replaceWith(0, 2, listified));
      return true;
   }, () => true)(mockState, mockDispatch)
...
export const splitParagraphs = (slice: Slice, schema: Schema) => {
   ...
   if (getNodeType(schema, 'paragraph').validContent(slice.content) && !hasCodeMark) {
   ...
   return mapSlice(slice, (node, _parent) => {
      if (node.type === getNodeType(schema, 'paragraph'))
...
export const upgradeTextToLists = (type: NodeType, slice: Slice, schema: Schema) => 
   mapSlice(slice, (node, _parent) =>
      node.type === getNodeType(schema, 'paragraph')
         ? extractListFromParagaph(type, node, schema)
         : node
   )
